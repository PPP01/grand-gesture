...
var sue = {
    ...
    handleEvent: e => {
        switch (e.type) {
            ...
            case "mouseup":
                if (
                    (e.button == 1 && config.mges.settings.model == 4) ||
                    (e.button == 2 &&
                        e.button == config.mges.settings.model &&
                        config.general.linux.cancelmenu &&
                        sue.cons.os != "win")
                ) {
                ...
                if (
                    !extDisable &&
                    config.general.fnswitch.fnrges &&
                ...
                break;
            case "contextmenu":
                if (config.general.linux.cancelmenu && sue.cons.os != "win") {
                    //fix mges
                    if (
                        config.general.fnswitch.fnmges &&
                        !sue.cons.fix_linux_value &&
                        config.mges.settings.model == 2
                    ) {
                        e.preventDefault();
                    }
                    ...
                    if (
                        config.general.fnswitch.fnrges &&
                        !sue.cons.fix_linux_value &&
                        config.rges.actions[1].name != "none"
                    ) {
                        e.preventDefault();
                    }
                    ...
                    //fix wges
                    if (
                        config.general.fnswitch.fnwges &&
                        !sue.cons.fix_linux_value &&
                        !(config.wges.actions[2].name == "none" && config.wges.settings[3].name == "none")
                    ) {
                        e.preventDefault();
                    }
                    ...
                    sue.cons.fix_linux_timer = window.setTimeout(() => {
                        sue.cons.fix_linux_value = false;
                    }, 500);
                    break;
                } else {
                    //all for win
            ...
            case "dragover":
                if (sue.drawing) {
                    ...
                    if (
                        !sue.cons.drginbox &&
                        config[sue.drawType[0]].settings.drgtobox &&
                        e.target &&
                        e.target.type &&
    ...
    ksa: {
        ...
        action: e => {
            ...
            for (var i = 0; i < config.ksa.actions.length; i++) {
                if (
                    key.ctrl == config.ksa.actions[i].ctrl &&
            ...
            sue.ksa.timeout = window.setTimeout(() => {
                sue.ksa.keyArray = [];
            }, config.ksa.settings.timeout);
        },
    },
    domCreate: (edom, eele, einner, ecss, edata, etxt) => {
    ...
    touchMove: e => {
        ...
        if (dx < config.general.settings.minlength && dy < config.general.settings.minlength) {
            return;
        }
        ...
        if (dir != lastDir) {
            ...
            //get tip
            config[sue.drawType[0]].ui.tip.enable || config[sue.drawType[0]].ui.note.enable
                ? sue.sendDir(sue._dirArray, "gettip", e)
                : null;
        }
        //timeout
    ...
    lineDrawReady: (e, type) => {
            ...
            switch (e.target.nodeType) {
                ...
                case 1:
                    if (e.target.src) {
                        ...
                        if (
                            config[type].settings.drgimg &&
                            e.target.firstElementChild &&
                            e.target.firstElementChild.nodeType == 1 &&
        ...
        if (
            type != "mges" &&
            ((config.general.fnswitch.fnsdrg && config.sdrg.settings.drgurl) ||
                (config.general.fnswitch.fndrg && config.drg.settings.drgurl))
        ) {
    ...
    UI: style => {
        ...
        if (!domui) {
            ...
            domui.style.cssText += "position:fixed;text-align:right;" + "z-index:" + Number.parseInt(new Date() / 1000);
            const objStyle = {
    ...
    ui_line: function (e) {
        if (!sue.document.querySelector("div[data-suui=line]")) {
            ...
            svgdiv.style.cssText +=
                ...
                "width:" +
                sue.window.innerWidth +
                "px;" +
                "height:" +
                sue.window.innerHeight +
                "px;" +
                "z-index:" +
            ...
            svgtag.style.cssText +=
                "width:" + sue.window.innerWidth + "px;" + "height:" + sue.window.innerHeight + "px;";
    ...
    ui_direct: e => {
        if (!config[sue.drawType[0]].ui.direct.enable) {
            return;
        }
        var uidom = sue.document.querySelector(
            "div[data-suui=uibox][data-sustyle=" + config[sue.drawType[0]].ui.direct.style + "]"
        );
        ...
        if (ui_dir) {
            ...
            _img.style.cssText +=
                "float:left;" +
                "height:" +
                config[sue.drawType[0]].ui.direct.width +
                "px;" +
                "transform:rotate(+" +
            ...
            ui_dir.style.cssText +=
                ...
                "background-color:" +
                config[sue.drawType[0]].ui.direct.color +
                " !important;" +
                "opacity:" +
                config[sue.drawType[0]].ui.direct.opacity / 100;
    ...
    ui_tip: (confOBJ, e) => {
        if (!config[sue.drawType[0]].ui.tip.enable) {
            return;
        }
        var uidom = sue.document.querySelector(
            "div[data-suui=uibox][data-sustyle=" + config[sue.drawType[0]].ui.tip.style + "]"
        );
        if (!uidom) {
            return;
        }
        ...
        if (!_dom) {
            ...
            _dom.style.cssText +=
                "display:inline-block;padding:0.1em 0.2em 0.2em 0.2em;border-radius: 0.15em;font-family: arial,sans-serif !important;" +
                "background-color:" +
                config[sue.drawType[0]].ui.tip.bgcolor +
                ";" +
                "color:" +
                config[sue.drawType[0]].ui.tip.color +
                ";" +
                "border: 2px solid " +
                config[sue.drawType[0]].ui.tip.color +
                ";" +
                "font-size:" +
                config[sue.drawType[0]].ui.tip.width +
                "px;" +
                "opacity:" +
                config[sue.drawType[0]].ui.tip.opacity / 100 +
                ";";
            ...
            if (config[sue.drawType[0]].ui.tip.withdir) {
                ...
                for (var i = 0; i < sue._dirArray.length; i++) {
                    var _dir = sue.domCreate(
                        ...
                        {
                            setName: ["src"],
                            setValue: [chrome.runtime.getURL("") + "image/direct.png"],
                        },
                        null,
    ...
    ui_note: (confOBJ, e) => {
        ...
        if (!_dom) {
            ...
            _dom.style.cssText +=
                "font-family: arial,sans-serif !important;font-style: italic;/*position:fixed;*/" +
                "color:" +
                config[sue.drawType[0]].ui.note.color +
                ";" +
                "font-size:" +
                config[sue.drawType[0]].ui.note.width +
                "px;" +
                "opacity:" +
                config[sue.drawType[0]].ui.note.opacity / 100 +
                ";";
            uidom.appendChild(_dom);
            var _br = document.createElement("br");
            uidom.appendChild(_br);
    ...
    ui_allaction: (confOBJ, e) => {
        ...
        if (!_dom) {
            ...
            _dom.style.cssText +=
                "font-family: arial,sans-serif !important;text-align:left;padding: 5px 20px;border-radius: 2px;" +
                "color:" +
                config[sue.drawType[0]].ui.allaction.color +
                ";" +
                "background-color:" +
                config[sue.drawType[0]].ui.allaction.bgcolor +
                ";" +
                "font-size:" +
                config[sue.drawType[0]].ui.allaction.width +
                "px;" +
                "opacity:" +
                config[sue.drawType[0]].ui.allaction.opacity / 100 +
                ";";
            uidom.appendChild(_dom);
        }
        _dom.textContent = "";
                ...
                for (var ii = 0; ii < confOBJ.allaction[i].direct.length; ii++) {
                    var _img = sue.domCreate(
                        ...
                        "vertical-align: text-top;height:" +
                            config[sue.drawType[0]].ui.allaction.width +
                            "px;transform:rotate(" +
                            sue.directimg(confOBJ.allaction[i].direct[ii]) +
                            ");"
                    );
    ...
    uiPos: e => {
        ...
        for (i = 0; i < domUIs.length; i++) {
            if (["center", "top", "ui_bottom", "left", "right"].includes(domUIs[i].dataset.sustyle)) {
    ...
    clearUI: () => {
        ...
        sue.document.querySelector("div[data-suui=line]")
            ? sue.document.querySelector("div[data-suui=line]").remove()
            : null;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type == "set_confirm") {
        ...
        sendResponse({ type: message.type, message: true });
    } else if (message.type == "icon" || message.type == "pop") {
        //icon action
        sue.selEle = {};
        sue.selEle.txt = window.getSelection().toString();
        // fix firefox get selection frome textbox
        ...
        extDisable = true;
        sendResponse({});
    } else if (message.type == "setapptype") {
        sue.appType[message.apptype] = true;
        sendResponse({});
    } else if (message.type == "fix_switchtab") {
        sue.cons.switchtab = {};
        sue.cons.switchtab.contextmenu = true;
        sendResponse({ type: message.type, message: true });
    } else if (message.type == "actionPaste") {
